...
class Alignment(object):
    ...
    def __init__(self, inputfile, roi_select, spec, firstimg, printmv,
        ...
        if printmv == 1:
            self.mv_vect_filename = inputfile.split('.hdf')[0]+'_shift'+'.txt'
    ...
    def findMvVector(self, coords_base_proj, coords_mv_proj):
    ...
    # Move a projection ###
    def mvProjection(self, empty_img, proj_two, mv_vector):
    ...
    def align_images(self):
        ...
        if self.spec == 0:
            ...
            #############################################
            try: 
                self.input_nexusfile.opendata('CurrentsTomo')
            ...
            ###################################################
            try: 
                self.input_nexusfile.opendata('ExpTimesTomo')
        ...
        if self.spec == 1:
            ...
            #############################################
            try: 
                self.input_nexusfile.opendata('Currents')
                currents = self.input_nexusfile.getdata()
                self.input_nexusfile.closedata()
                self.align['Currents'] = currents
                self.align['Currents'].write()
        ...
        self.nxsfied = self.align[self.data_nxs]
        self.writeImageInHdf5(self.image_proj1, self.nxsfied, slab_offset)
        ...
        if self.user_roi_select == 0:
            self.roi_points[0] = self.roi_points.append((self.col_tem_from,
                                                         self.row_tem_from))
            self.roi_points[1] = self.roi_points.append((self.col_tem_to,
                                                         self.row_tem_to))
